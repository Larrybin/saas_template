...
4. ✅ 收敛 plan/credits 策略：已引入 `PlanCreditsPolicy`（`src/credits/domain/plan-credits-policy.ts`），由其读取 `websiteConfig` 派生注册赠送 / 免费月刷新 / 续费 / 终身逻辑；`CreditLedgerService` 仅依赖该抽象，并通过构造注入使用默认策略实例。
5. ✅ 为 `useCurrentPlan` 等复合 Hook 提取共享的 domain helper：已在 `src/domain/plan/resolve-current-plan.ts` 实现 `resolveCurrentPlan(subscription, isLifetimeMember, plans)` 纯函数，并在 `useCurrentPlan` 中复用；补充了覆盖 lifetime / subscription / free / disabled 的针对性单元测试。
