...
5. **后续建议**
   ...
   - **前端日志统一方案**（详述如下）：
     1. 新增 `src/lib/client-logger.ts`，暴露 `info/warn/error/debug` 等方法；内部根据 `process.env.NODE_ENV` 判断是否输出到 `console`，并预留扩展点（未来可接入 Sentry/LogRocket 等）。生产模式仅在显式设置 `NEXT_PUBLIC_ENABLE_CLIENT_LOGGER=true` 时输出，以避免敏感信息泄露。2025-11-24 已完成落库。
     2. 将 `logAnalyzerComponentError`、`logError`、`PerformanceMonitor`、AI hooks（`useWebContentAnalyzer`、`useImageGeneration`）以及 credits 模块公共日志替换为 `clientLogger`，同步移除 Image 模块与 Analyzer 中的大部分裸 `console.*`。2025-11-24 已扩展至 `components/auth/*`、`components/settings/*`、`components/layout/*`（user button/Crisp）、`components/pricing/*`、`components/contact/*`、`components/newsletter/*`、`components/waitlist/*`、`components/admin/*` 以及 `notification/*`、`lib/user-lifecycle/*` 等关键链路。
     3. 持续清理各组件中的 `console.*`：对需保留的调试输出改用 `clientLogger.debug`，其余直接删除或改为 toast/日志。当前已覆盖 AI/credits、auth、settings、layout/pricing/notification，剩余 server actions 与 legacy 脚本将结合场景使用 `getLogger` 或专用 CLI logger。
     4. 在 Readme 记录“前端禁止直接使用 console，统一通过 `clientLogger`”的约定，并在 Code Review 中执行；未来若接入 Sentry，只需在 `client-logger.ts` 接管实现即可完成全局接入。
