...
| Server Action 缺少 `code`，无法复用错误 UI 策略 | `src/actions/subscribe-newsletter.ts` 等 | P1 | 1.5 人日 | **Billing/Credits + Newsletter + Contact/Captcha 已按 `.codex/plan/server-actions-domain-error-unification.md` 完成，通过 DomainError + ErrorCodes 统一返回；其余 Actions 仍待盘点与治理。** |
