...
export interface ExpirationJobDeps {
  creditLedgerDomainService: CreditLedgerDomainService;
}

const defaultDeps: ExpirationJobDeps = {
  creditLedgerDomainService: new CreditLedgerDomainService(
...
export function createRunExpirationJob(deps: ExpirationJobDeps) {
  const { creditLedgerDomainService } = deps;

  return async function runExpirationJob(options?: {
    batchSize?: number;
...
export async function runExpirationJob(options?: {
  ...
  const run = createRunExpirationJob(defaultDeps);
  return run(options);
}
