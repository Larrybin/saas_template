...
export function ImageDisplay({
  ...
  const isInteractive = Boolean(image && !failed);
  ...
  return (
    <>
        ...
        {image && !failed ? (
          <div className="relative h-full w-full">
            <button
              type="button"
              onClick={openZoom}
              className="relative block h-full w-full rounded-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary/70"
              aria-label={`View the image generated by ${provider}`}
            ...
            <Button
              size="icon"
              variant="secondary"
              className="absolute bottom-2 left-2 z-20 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity"
      ...
      {isZoomed &&
        ...
        createPortal(
          <div
            className="fixed inset-0 bg-black/90 z-50 flex items-center justify-center cursor-pointer min-h-dvh w-screen"
            ...
            role="dialog"
            aria-modal="true"
            aria-label={`Generated image preview from ${provider}`}
            tabIndex={-1}
          >
            <div className="relative max-h-[90dvh] max-w-[90vw] w-full aspect-square">
              <Image
                src={`data:image/png;base64,${image}`}
                alt={`Generated by ${provider}`}
                fill
                sizes="(max-width: 1536px) 90vw, 768px"
