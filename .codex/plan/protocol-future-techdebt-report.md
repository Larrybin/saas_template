...
| - | -------- | -------- | ---- | ------ | ---- | ---- |
| 1 | 协议一致性 / API | 新增 `/api/*` 路由缺少自动化 Envelope 校验，依赖人工 review | `docs/error-logging.md` 仅给出模式示例；`src/app/api/*` 中 Envelope 实现依靠手写 | P1 | 2d | 建议编写脚本扫描 route 文件，检测缺失 `success` 字段或非标准错误结构，并在 CI 中执行 |
...
| 4 | ErrorCodes ↔ 文档/前端 映射 | ErrorCodes 中部分码在 docs 或前端 registry 中缺少对应条目，影响观测与 UI 一致性 | `src/lib/server/error-codes.ts` vs `docs/error-codes.md` vs `domain-error-ui-registry.ts` | P1 | 2d | 建议编写脚本列出未文档化或未使用的错误码，并在 docs 中补齐或清理 |
| 2 | 协议一致性 / Actions | safe-action 规范未有静态守护，新 Actions 仍可能绕过 DomainError 模型 | `src/actions/*` 当前已统一，但缺少 lint/规则约束；`docs/error-logging.md` 仅描述推荐模式 | P1 | 1.5d | 可通过 AST/regex 扫描，确保所有 Actions 通过 `createSafeActionClient` 并在错误时返回 `code`/`retryable`，与架构体检报告 P1「收敛 action 层错误处理模式」保持一致 |
| 5 | AI 计费配置缺少 plan/region 维度 | AI 计费规则主要通过 `billing-config.ts` 常量表达，未与 plan/region 显式关联 | `src/ai/billing-config.ts` + `docs/ai-lifecycle.md` | P1 | 2d | 与架构体检报告 P0「抽象 AI 计费策略注入接口」属于同一类工作；建议通过 `AiBillingRuleRepository` 或扩展 PlanPolicy，将 plan/region 纳入规则生成入口 |
| 6 | Payment 多 Provider / 多租户 支撑不足 | Payment 组合根仍假定单 Stripe 实例，缺乏对多 Provider / 租户的抽象 | `docs/payment-lifecycle.md` + `src/payment/services/stripe-payment-*` | P1 | 3d | 与架构体检报告 P1「为 payment / AI / mail 引入轻量级配置 provider」配套，建议引入 `PaymentProviderFactory` 以 tenant/region 为输入返回 Provider/服务实例 |
...
| 11 | 协议与错误模型未有 CI 级守护 | Envelope/错误码/spans 主要靠人工检查 | `docs/error-logging.md` + CI 配置 | P1 | 2d | 结合 #1/#2/#4/#15，通过一组脚本在 CI 中对协议/错误模型做基础检查 |
| 12 | Error Logging 表与实际 span 使用可能漂移 | `docs/error-logging.md` 中 span 汇总表需手动维护 | `docs/error-logging.md` vs `src/lib/server/logger.ts` 使用情况 | P2 | 1.5d | 脚本扫描代码中的 span 常量并与 docs 表对比，提示未登记或过期的 span |
